#!/bin/bash
#SBATCH --job-name=ptv3_ae_sinr_aux_4gpu
...
#SBATCH --mem=128G
#SBATCH --output=ptv3_ctx_ae_sinr_cat_aux_4gpu_200ep_10class_dual_val.%j.out
#SBATCH --error=ptv3_ctx_ae_sinr_cat_aux_4gpu_200ep_10class_dual_val.%j.err
...
CONFIG=$WD/configs/standardized_dataset/cls-ptv3-ctx-ae-sinr-cat-aux-10class-dual-val-4gpu.py
EXP_DIR=$WD/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_cat_aux_4gpu_200ep
...
apptainer exec --nv --pwd "$WD" \
  ...
  python -u tools/train.py \
    --config-file "$CONFIG" \
    --num-gpus 4 \
    --options save_path="$EXP_DIR" epoch=120 eval_epoch=120 num_worker=16
