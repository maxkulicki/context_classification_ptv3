#!/bin/bash
#SBATCH --job-name=ptv3_small_4gpu_100ep_3class
...
#SBATCH --mem=128G
#SBATCH --output=ptv3_small_4gpu_100ep_3class.%j.out
#SBATCH --error=ptv3_small_4gpu_100ep_3class.%j.err
...
CONFIG=$WD/configs/standardized_dataset/cls-ptv3-baseline-genus-small-3class.py
EXP_DIR=$WD/exp/snapshot_3class_mixed/ptv3_small_4gpu_100ep
...
apptainer exec --nv --pwd "$WD" \
  ...
  python -u tools/train.py \
    --config-file "$CONFIG" \
    --num-gpus 4 \
    --options save_path="$EXP_DIR" epoch=100 eval_epoch=100 num_worker=16 batch_size=128 batch_size_val=256
