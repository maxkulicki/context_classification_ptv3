"""
Build context_features_sinr_hc1000.pth from:
  - data/snapshot_v1/features.csv        — AE, topo, GPN columns
  - data/snapshot_v1/sinr_features_hc1000.csv — SINR cols extracted with
...
Run extraction first (from context_classification_ptv3/):
    conda run -n context_baseline python extract_sinr_features.py \
        ...
        --output data/snapshot_v1/sinr_features_hc1000.csv \
        --model sinr/pretrained_models/model_an_full_input_enc_sin_cos_hard_cap_num_per_class_1000.pt

Then build the .pth (from context_classification_ptv3/):
    conda run -n context_baseline python preprocess_context_features_sinr_hc1000.py

Output:
    data/snapshot_v1/context_features_sinr_hc1000.pth
"""
...
SINR_HC1000_CSV = "data/snapshot_v1/sinr_features_hc1000.csv"
OUT_PATH      = "data/snapshot_v1/context_features_sinr_hc1000.pth"

df_feat = pd.read_csv(FEATURES_CSV)
df_sinr = pd.read_csv(SINR_HC1000_CSV)
...
print(f"SINR (hc1000): {len(sinr_cols)} cols")
print(f"GPN:  {len(gpn_cols)} cols")
print(f"features.csv rows: {len(df_feat)}")
print(f"sinr_hc1000 rows:  {len(df_sinr)}")

# Build stem→sinr lookup from hc1000 CSV
sinr_by_stem = {}
...
if missing_sinr:
    print(f"WARNING: {missing_sinr} trees had no hc1000 SINR entry — zeroed out")
