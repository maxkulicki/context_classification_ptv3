    ...
    def after_epoch(self):
        if is_main_process():
            ...
            if self.trainer.cfg.evaluate:
                ...
                # Track best mAcc separately
                current_mAcc = self.trainer.comm_info.get("current_mAcc_value")
                if current_mAcc is not None:
                    if current_mAcc > self.best_mAcc_value:
                        self.best_mAcc_value = current_mAcc
                        is_best_mAcc = True
                        self.trainer.logger.info(
                            "Best validation mAcc updated to: {:.4f}".format(
                                current_mAcc
                            )
                        )
                    self.trainer.logger.info(
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            for key, value in checkpoint["state_dict"].items():
                ...
                weight[key] = value
            # Filter out keys with shape mismatches (e.g. classification
            # head when num_classes differs between pretrained and current).
            model_state = self.trainer.model.state_dict()
            skipped = []
            for key in list(weight.keys()):
                if key in model_state and weight[key].shape != model_state[key].shape:
                    skipped.append(key)
                    del weight[key]
            if skipped:
                self.trainer.logger.info(
                    f"Skipped keys with shape mismatch: {skipped}"
                )
