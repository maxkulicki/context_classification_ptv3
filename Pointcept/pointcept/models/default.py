...
class ContextClassifier(nn.Module):
    """Classifier with late fusion of PTv3 backbone features and context embeddings.

    Two fusion modes:
    - "projected": Project both streams to equal dimensions before concatenation,
      balancing the 512 vs 64 imbalance.
    - "direct": Concatenate 512 + 64 = 576 directly, let the MLP figure it out.
    """
    ...
    def __init__(
        ...
        if fusion_mode == "projected":
            ...
            fused_dim = backbone_embed_dim + context_dim
    ...
    def forward(self, input_dict):
        ...
        # Mean pool point features -> [B, backbone_embed_dim]
        if isinstance(point, Point):
            point.feat = torch_scatter.segment_csr(
                src=point.feat,
                indptr=nn.functional.pad(point.offset, (1, 0)),
