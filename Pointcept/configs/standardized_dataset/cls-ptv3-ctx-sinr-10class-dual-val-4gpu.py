"""
PTv3 + SINR context fusion — 10-class dual-val dataset, 4-GPU / 100-epoch.
  Late concat fusion: PTv3 (512) + SINREncoder (256) → 768 → num_classes
  Architecture mirrors cls-ptv3-ctx-ae-10class-dual-val-4gpu.py, with SINR
  replacing AlphaEarth:
    - SINREncoder(input_dim=256, output_dim=256) instead of AEEncoder(64→256)
    - context_sources=['sinr'], key='ctx_sinr'
  Dataset, loss, backbone, optimizer identical to the AE 4-GPU config.
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_sinr_4gpu_100ep'
num_worker = 16
batch_size = 128
