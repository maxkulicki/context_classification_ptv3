"""
PTv3 + AlphaEarth + SINR concat fusion with auxiliary heads — 10-class dual-val, 4-GPU / 200-epoch.
  Late concat fusion: PTv3 (512) + AEEncoder (256) + SINREncoder (256) → 1024 → num_classes
  Architecture: MultiCatCtxCls-v1m2 — both sources always active, simple concat,
  plus per-branch auxiliary classification heads (lidar, ctx_ae, ctx_sinr) with
  aux_loss_weight=0.25 to ensure each encoder receives direct gradient.
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_cat_aux_4gpu_200ep'
num_worker = 16
batch_size = 128
