...
# Phase 1, Experiment 1.4: PTv3 + GeoPlantNet (18-dim species logit scores)
# Late concat fusion: PTv3 (512) + GPNEncoder (256) → 768 → 512 → num_classes
# Encoder applies log(clamp(x, 1e-6)) to compress score range.
# NaN values (outside GPN coverage) are zeroed in the dataset → map to log(1e-6) ≈ -13.8
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            dict(
                type="Collect",
                keys=("coord", "grid_coord", "category", "ctx_gpn"),
                feat_keys=["coord"],
            ),
        ],
        test_mode=False,
