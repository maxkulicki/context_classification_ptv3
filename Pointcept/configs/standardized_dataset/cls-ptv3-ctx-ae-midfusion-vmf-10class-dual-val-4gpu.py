"""
PTv3 + AlphaEarth mid-fusion with vMF augmentation — 10-class dual-val, 4-GPU / 120-epoch.
  AE embedding (256-d) injected after PTv3 encoder stage 3 (256-d features),
  before the final bottleneck stage (enc4 → 512-d). Concat + Linear projection
  preserves the enc3 channel dim so enc4 runs unmodified.
  No aux heads. CtxVMFAugment (kappa=100) applied to ctx_ae during training.
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_midfusion_vmf_4gpu_120ep'
num_worker = 16
batch_size = 128
