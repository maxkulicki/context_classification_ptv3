...
# Phase 2, Experiment 2.1: PTv3 + AE + SINR, simple deterministic concat fusion.
# No attention, no source dropout, no aux heads, no Lovász.
# fused_dim = 512 (backbone) + 256 (AE) + 256 (SINR) = 1024 → 512 → num_classes
# Backbone pretrained from ptv3_baseline_8gpu_200ep, fine-tuned at lower LR.
# Baseline comparisons: Exp 1.0 (no context), Exp 1.1 (AE only), Exp 1.3 (SINR only)

# misc custom setting
batch_size = 64  # total across all GPUs (16 per GPU × 4 GPUs)
num_worker = 16
batch_size_val = 256
empty_cache = False
enable_amp = True
...
# backbone at 10× lower LR than context encoders + cls_head
    ...
    test=dict(
        ...
        test_cfg=dict(
            post_transform=[
                ...
                dict(
                    type="Collect",
                    keys=("coord", "grid_coord", "ctx_ae", "ctx_sinr"),
                    feat_keys=["coord"],
                ),
            ],
            aug_transform=[
