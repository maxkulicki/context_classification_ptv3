"""
PTv3 + AE + SINR concat fusion, aux heads (weight=0.5), full AE augmentation — 10-class dual-val, 4-GPU / 120-epoch.
  Architecture: MultiCatCtxCls-v1m2
    Late concat: PTv3 (512) + AEEncoder (256) + SINREncoder (256) → 1024 → num_classes
    Auxiliary classification heads on each branch, aux_loss_weight=0.5.
    No modality dropout.
  AE augmentation: CtxPoolSample (spatial+temporal pool, p_drop=0.2) + CtxVMFAugment (kappa=100).
  SINR augmentation: none.
  Uses context_features_combined_aug.pth (has ctx_ae + ctx_ae_pool per stem).
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_cat_aux05_combaug_4gpu_120ep'
num_worker = 16
batch_size = 128
...
# .pth produced by merge_context_features_combined_aug_sinr.py
# format per stem: {"ctx_ae": (64,), "ctx_ae_pool": (50, 64), "ctx_sinr": (256,)}
context_pth = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/data/snapshot_v1/context_features_combined_aug_sinr.pth'
cache_data = False
...
data = dict(
    ...
    train=dict(
        ...
        context_sources=['alphaearth', 'alphaearth_pool', 'sinr'],
        transform=[
