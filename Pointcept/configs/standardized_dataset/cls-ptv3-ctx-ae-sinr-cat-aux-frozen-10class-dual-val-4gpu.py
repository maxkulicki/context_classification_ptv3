"""
PTv3 (frozen pretrained) + AE + SINR concat fusion with aux heads — 10-class dual-val, 4-GPU / 50-epoch.
  Backbone loaded from baseline checkpoint and frozen. Only context encoders,
  fusion head, and auxiliary heads are trained.
  Late concat fusion: PTv3 (512) + AEEncoder (256) + SINREncoder (256) → 1024 → num_classes
  Architecture: MultiCatCtxCls-v1m2 with freeze_backbone=True.
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_cat_aux_frozen_4gpu_50ep'
num_worker = 16
batch_size = 128
...
wandb_key = None
mix_prob = 0
# No param_dicts — backbone is frozen, all trainable params share the same LR
param_dicts = None
...
_backbone_weight = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_small_4gpu_100ep/model/model_best_mAcc.pth'
...
model = dict(
    ...
    backbone_weight=_backbone_weight,
