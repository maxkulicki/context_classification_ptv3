"""
PTv3 + AE + SINR concat fusion, aux heads (weight=0.5) + SINR Gaussian noise augmentation
  — 10-class dual-val, 4-GPU / 120-epoch.
  Architecture: MultiCatCtxCls-v1m2
    ...
    No modality dropout.
  Training augmentation: CtxVMFAugment on ctx_ae (kappa=100) +
                         CtxGaussianNoise on ctx_sinr (sigma=0.1, p_drop=0.2).
  Otherwise identical to cls-ptv3-ctx-ae-sinr-cat-aux05-10class-dual-val-4gpu-120ep.py.
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_cat_aux05_sinrgauss_4gpu_120ep'
num_worker = 16
batch_size = 128
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            dict(type='CtxGaussianNoise', key='ctx_sinr', sigma=0.1, p_drop=0.2),
            dict(type='Collect',
