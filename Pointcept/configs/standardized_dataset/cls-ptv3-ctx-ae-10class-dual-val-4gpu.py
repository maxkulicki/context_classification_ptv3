"""
PTv3 + AlphaEarth context fusion — 10-class dual-val dataset, 4-GPU / 100-epoch.
  Late concat fusion: PTv3 (512) + AEEncoder (256) → 768 → num_classes
  Architecture mirrors cls-ptv3-ctx-ae-snapshot1-genus.py, adapted for:
    - snapshot_10class_dual_val (70/15/15 train/val_id/val_ood)
    - 10 genera (Abies Acer Alnus Betula Carpinus Fagus Larix Picea Pinus Quercus)
    - DualValClsEvaluator for separate val_id / val_ood tracking
  context_features.pth from snapshot_v1 covers the same trees (same sources).
Scaling vs 8-GPU config:
  batch_size 256→128 (32/GPU), batch_size_val 512→256
  LR 0.008→0.004 (halved with batch size; avoids AdamW linear-scaling overshoot)
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_4gpu_100ep'
num_worker = 16
batch_size = 128
