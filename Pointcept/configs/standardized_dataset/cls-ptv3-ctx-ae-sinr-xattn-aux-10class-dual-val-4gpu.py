"""
PTv3 + AlphaEarth + SINR cross-attention fusion with auxiliary heads — 10-class dual-val, 4-GPU / 200-epoch.
  Cross-attention fusion: PTv3 features (512) attend to context embeddings
  (AE 256, SINR 256) as keys/values. Residual + FFN preserves backbone dim.
  Architecture: MultiXAttnCtxCls-v1m1
  Per-branch auxiliary heads (lidar, ctx_ae, ctx_sinr), aux_loss_weight=0.25.
  CtxVMFAugment (kappa=100) applied to ctx_ae during training.
  From-scratch training, nothing frozen.
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_xattn_aux_4gpu_200ep'
num_worker = 16
batch_size = 128
