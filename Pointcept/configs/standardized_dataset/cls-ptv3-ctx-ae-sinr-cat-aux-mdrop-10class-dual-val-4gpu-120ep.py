"""
PTv3 + AE + SINR concat fusion, aux heads + modality dropout — 10-class dual-val, 4-GPU / 120-epoch.
  Architecture: MultiCatCtxCls-v1m3
    Late concat: PTv3 (512) + AEEncoder (256) + SINREncoder (256) → 1024 → num_classes
    Auxiliary classification heads on each branch (aux_loss_weight=0.25).
    Modality dropout: during training each sample independently drops one context
    source with p=0.25 (zeroed before the main head; aux heads always see full embeddings).
  Otherwise identical to cls-ptv3-ctx-ae-sinr-cat-aux-10class-dual-val-4gpu.py.
"""
...
test_only = False
seed = 3313067
save_path = '/net/pr2/projects/plgrid/plggtreeseg/context_classification_ptv3/Pointcept/exp/snapshot_10class_dual_val/ptv3_ctx_ae_sinr_cat_aux_mdrop_4gpu_120ep'
num_worker = 16
batch_size = 128
